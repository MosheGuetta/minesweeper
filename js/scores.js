...
let gScores = {
  ...
  ManualMode: Array.isArray(JSON.parse(localStorage.getItem('scoreManual'))) ? 
  JSON.parse(localStorage.getItem('scoreManual')) : [],
};
...
function updateScoreTable() {
  ...
  const manualScores = gScores.ManualMode.sort((a, b) => b.score - a.score).slice(0, 10); // Added for Manual Mode
  ...
  for (let i = 0; i < Math.max(easyScores.length, mediumScores.length, hardScores.length, manualScores.length); i++) {
      ...
      const manualRow = manualScores[i] ? `
          ...
          : `<td></td><td></td>`; // Added for Manual Mode
      ...
      const rowHTML = `
          <tr>
              ...
              ${mediumRow}
              ${hardRow}
              ${manualRow} <!-- Added for Manual Mode -->
          </tr>
      `;
...
function saveScore(level, playerName) {
  ...
  } else if (level === 'ManualMode') { // Added for Manual Mode
      gScores.ManualMode.push(newScoreEntry);
      localStorage.setItem('scoreManual', JSON.stringify(gScores.ManualMode));
  }
